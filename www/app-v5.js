...
function generateCode({ showErrors = false } = {}) {
  ...
  if (!phone || !amountRaw) {
    if (showErrors) {
      ...
      if (!phone) setFieldError(phoneError, '');
      if (!amountRaw) setFieldError(amountError, '');
    }
    phoneInput.classList.toggle('input-invalid', !phone && showErrors);
    amountInput.classList.toggle('input-invalid', !amountRaw && showErrors);
    return false;
  }

  let valid = true;
  if (!validatePhone(phone)) {
    if (showErrors) setFieldError(phoneError, 'رقم الجوال يجب أن يكون 10 خانات ويبدأ بـ 059 أو 056.');
    phoneInput.classList.toggle('input-invalid', showErrors);
    valid = false;
  } else {
  ...
  if (!validateAmount(amountRaw)) {
    if (showErrors) setFieldError(amountError, 'المبلغ يجب أن يكون من 1 إلى 1000، ويمكن أن يحتوي على فاصلة عشرية.');
    amountInput.classList.toggle('input-invalid', showErrors);
    valid = false;
  } else {
...
function detectDetailsDirection(value = '') {
  const text = String(value || '').trim();
  const firstStrong = text.match(/[A-Za-z\u0590-\u08FF]/);
  if (!firstStrong) return 'rtl';
  return /[A-Za-z]/.test(firstStrong[0]) ? 'ltr' : 'rtl';
}

function applyDetailsDirection(textarea) {
  const direction = detectDetailsDirection(textarea.value);
  textarea.setAttribute('dir', direction);
  textarea.classList.toggle('details-ltr', direction === 'ltr');
  textarea.classList.toggle('details-rtl', direction === 'rtl');
}
...
function renderRecords() {
  ...
  filtered.forEach((record, index) => {
    ...
    tr.innerHTML = `
      ...
      <td><textarea class="details-input details-rtl" dir="rtl" lang="ar" inputmode="text" rows="1" enterkeyhint="done" data-id="${record.id}" placeholder="اكتب ماذا اشتريت...">${escapeHtml(record.details || '')}</textarea></td>
      <td>
...
function bindRecordEvents() {
  document.querySelectorAll('.details-input').forEach(input => {
    applyDetailsDirection(input);
    resizeDetailsInput(input);
    input.addEventListener('input', e => {
      applyDetailsDirection(e.target);
